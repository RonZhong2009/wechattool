...
def text_reply(msg):
#我们是幸福的一家*^o^*
    if msg['User']['NickName'] == '我们是幸福的一家*^o^*':
        ...
        if match:
            ...
            itchat.send('%s，%s' % (username, REPLY['工作'][randomIdx]), msg['FromUserName'])
        ...
        if match:
            ...
            itchat.send('%s，%s' % (username, REPLY['学习'][randomIdx]), msg['FromUserName'])
			
        match = re.search('味道', msg['Text']) or re.search('吃', msg['Text']) or  re.search('饭', msg['Text']) or re.search('餐', msg['Text'])
        if match:
            print('-+-+' * 5)
            print('Message content:%s' % msg['Content'])
            print('味道、吃 is: %s' % (match is not None))
            randomIdx = random.randint(0, len(REPLY['味道']) - 1)
            itchat.send('%s，%s' % (username, REPLY['味道'][randomIdx]), msg['FromUserName'])
